 ...
 *
 *  @param first the index of the first element of the sublist
 *  @param last the index of the last element of the sublist
 *  @return the index of the middle of the list
 */
int recursive_middle_search(int first, int last){
	int n_first = first;
	int n_last = last;
	
	if(inicio == livre){//Empty list
		return -1;
	}
	
	if(first == -1 && last == -1){//Just starting the search
		n_first = inicio;
		n_last = get_idx_from_last();
	}
	
	if(n_first == n_last || n_first == get_previous(n_last))
		return n_first;
	return recursive_middle_search(v[n_first].prox, get_previous(n_last));
}
